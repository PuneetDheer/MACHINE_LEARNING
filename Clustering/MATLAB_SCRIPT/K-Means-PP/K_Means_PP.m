...
% Cluster_ind = cluster number of all data points
...
% 
function [Euclid_dist, within_Cluster_dist, Cluster_ind, NEWcentroid, Centroid_distance] = K_Means_PP(data, No_of_cluster, Dimension)

OLDcentroid(1,:) = data(randi([1,size(data,1)],1,1),:);

for i = 1:No_of_cluster-1
    
    D_error = bsxfun(@minus,data,OLDcentroid(i,:));
    
    D2(:,i) = sum(D_error.^2,2);
    
    [~,large_Dist] = max(min(D2,[],2)./sum(min(D2,[],2)));
    
    OLDcentroid(i+1,:) = data(large_Dist,:);
